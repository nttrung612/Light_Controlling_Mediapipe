...
def is_handsign_character(c):
    return ord("a") <= ord(c) < ord("q") or c == " "

def label_dict_from_config_file(path):
...
def run(data_path, sign_img_path, split="val", resolution=((1280, 720))):
    ...
    while cam.isOpened():
        ...
        key = cv2.waitKey(1)  # Đợi phím nhấn
        if key == -1:  # Nếu không có phím nhấn, không làm gì
            if current_letter is None:
                pass
            else:
                if len(hands):
            ...
            if cannot_switch_char:
                cv2.putText(annotated_image, f"please press {current_letter} again to unbind", (0, 450), cv2.FONT_HERSHEY_SIMPLEX,
                            1, (0, 255, 0), 2, cv2.LINE_AA)

        cv2.putText(annotated_image, status_text, (5, 20), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
        cv2.imshow(f"{split}", annotated_image)  # Hiển thị khung hình từ camera

    # Đảm bảo sau khi vòng lặp kết thúc, bạn chỉ đóng cửa sổ khi nhấn 'q'
...
if __name__ == "__main__":
    ...
    run(data_path, sign_img_path, split='train', resolution=(1280, 720))
    run(data_path, sign_img_path, split='val', resolution=(1280, 720))
    run(data_path, sign_img_path, split='test', resolution=(1280, 720))